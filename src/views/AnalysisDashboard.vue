...
import { BarChart3, TrendingUp, AlertTriangle, CheckCircle2, BookOpen, Sparkles } from 'lucide-vue-next'
...
onMounted(async () => {
  ...
  try {
    ...
    if (projectId.value) {
      const fores = await foreshadowApi.list(projectId.value).catch(() => [])
      foreshadows.value = Array.isArray(fores) ? fores : (fores?.foreshadows || [])
      analysisApi.dashboard(projectId.value).then((r: any) => { analysisData.value = r }).catch(() => {})
    }
  } catch (e) { /* ignore */ }
  loading.value = false
})

const totalWords = computed(() => chapters.value.reduce((sum: number, c: any) => sum + ((c.content || '').length), 0))
const hookStrength = computed(() => Math.min(100, chapters.value.reduce((s: number, c: any) => s + ((c.content || '').match(/为什么|怎么|难道|究竟|没想到|居然|秘密|真相|多年以后|后来/g || []).length, 0) * 8 + (chapters.value.length ? 10 : 0))
...
const unrecovered = computed(() => foreshadows.value.filter((f: any) => f.status !== 'recovered').length
</script>

<template>
  <div class="page">
    <div class="page-header">
      <h1><BarChart3 /> 追读力分析</h1>
      <p class="subtitle">Hook 强度 · 爽点密度 · 伏笔回收 · 章节节奏</p>
      <el-tag v-if="project" type="primary" size="small" style="margin-left:8px">作品：{{ project.name }}</el-tag>
    </div>

    <el-row :gutter="20" v-if="!loading">
      <el-col :span="6">
        <div class="metric-card a"><div class="metric-label">综合追读力</div>
      ...
      <el-col :span="6">
        <div class="metric-card b"><div class="metric-label">Hook 强度</div><div class="metric-value">{{ hookStrength }}</div><small>悬念/反转/疑问词</small></div>
      </el-col>
      <el-col :span="6">
        <div class="metric-card c"><div class="metric-label">章节 / 字数</div><div class="metric-value">{{ chapters.length }}</div><small>{{ totalWords.toLocaleString() }} 字</small></div>
      </el-col>
      <el-col :span="6">
        <div class="metric-card d"><div class="metric-label">未回收伏笔</div><div class="metric-value warn">{{ unrecovered }}</div><small>共 {{ foreshadows.length }} 条</small></div>
      </el-col>
    </el-row>

    <el-card v-if="!loading && project" class="mt-20" shadow="never">
      <template #header><div class="card-header"><span><Sparkles /> 写作诊断与建议</span></div></template>
      <el-alert v-if="hookStrength < 60" type="warning" :closable="false" title="💡 Hook 强度偏低：建议在关键情节加入悬念、反转或角色的疑问（为什么？怎么回事？难道…），让读者产生强烈好奇心。" />
      <el-alert v-if="chapters.length < 3" type="info" :closable="false" title="📖 章节数较少：完成至少 3 章后才能进行更稳定的节奏分析。" />
      <el-alert v-if="unrecovered > 0" type="danger" :closable="false" title="⚠️ 情节债务：有 {{ unrecovered }} 条伏笔尚未回收，建议在后续章节安排回收，避免读者失望。" />
      <el-alert v-if="composite >= 80" type="success" :closable="false" title="🎉 追读力优秀：整体节奏感良好，继续保持当前节奏推进主线。" />
    </el-card>

    <el-card v-if="!loading && chapters.length" class="mt-20" shadow="never">
      <template #header><div class="card-header"><span><BookOpen /> 章节列表</span></div></template>
      <el-table :data="chapters" stripe>
        ...
        <el-table-column label="结构" width="120">
          <template #default="{ row }"><el-tag size="small" :type="((row.content || '').length > 2000 ? 'success' : 'info'">{{ ((row.content || '').length > 2000) ? '合格' : '偏短' }}</el-tag></template>
        </el-table-column>
        <el-table-column prop="updatedAt" label="最近更新" width="200" />
      </el-table>
    </el-card>

    <el-empty v-if="!loading && !project" description="请先创建一个作品" />
  </div>
</template>
